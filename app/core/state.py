...
class AppState:
    def __init__(self, db_path: str | None = None) -> None:
        ...
        chroma_dir = os.environ.get("DATA_DIR", "./data")
        resolved_db = db_path or os.path.join(chroma_dir, "user_state.db")
        self._vdb = ChromaVectorDB(persist_dir=os.path.join(chroma_dir, "chroma"))
    ...
    ) -> Optional[IngestedPaper]:
        uid = user_id or get_current_user_id()
        return self._get_store(uid).get_paper(paper_id)

    def add_paper(
        self, paper_id: str, paper: IngestedPaper, user_id: Optional[str] = None
    ) -> None:
        uid = user_id or get_current_user_id()
    ...
    def add_sections(
        self, paper_id: str, sections: dict[str, str], user_id: Optional[str] = None
    ) -> None:
        uid = user_id or get_current_user_id()
    ...
    def add_chunks(
        self, paper_id: str, chunks: list[Chunk], user_id: Optional[str] = None
    ) -> None:
        uid = user_id or get_current_user_id()
    ...
    def add_embedding(
        self, paper_id: str, embedding: list[float], user_id: Optional[str] = None
    ) -> None:
        uid = user_id or get_current_user_id()
